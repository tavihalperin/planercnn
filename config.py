...
class Config(object):
    ...
    def __init__(self, options):
        ...
        with torch.no_grad():
            self.URANGE_UNIT = ((torch.arange(self.IMAGE_MAX_DIM, requires_grad=False).to(
                device).float() + 0.5) / self.IMAGE_MAX_DIM).view((1, -1)).repeat(
                self.IMAGE_MIN_DIM, 1)
            self.VRANGE_UNIT = ((torch.arange(self.IMAGE_MIN_DIM, requires_grad=False).to(
                device).float() + 0.5) / self.IMAGE_MIN_DIM).view((-1, 1)).repeat(1,
                                                                                self.IMAGE_MAX_DIM)
