...
class RefinementNet(nn.Module):
    ...
    def forward(self, image_1, camera, prev_result):
        ...
        all_masks_one_hot = (all_masks.max(0, keepdim=True)[1] == torch.arange(len(all_masks)).to(device).long().view(-1, 1, 1)).float()
...
class RefinementNetConcat(nn.Module):
    ...
    def forward(self, image_1, camera, prev_result):
        ...
        if num_planes < max_num_planes:
            masks = torch.cat([masks, torch.zeros((1, max_num_planes - num_planes, int(masks.shape[2]), int(masks.shape[3]))).to(device)], dim=1)
            pass
...
class RefineModel(nn.Module):
    def __init__(self, options):
        ...
        K = [[0.89115971,  0,  0.5],
             ...
             [0,           0,    1]]
        with torch.no_grad():
            self.intrinsics = torch.Tensor(K).to(device)
            pass
        """ the whole network """
