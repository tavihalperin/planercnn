...
def pyramid_roi_align(inputs, pool_size, image_shape):
    ...
    for i, level in enumerate(range(2, 6)):
        ...
        pooled_features = torchvision.ops.roi_align(
            feature_maps[i], [level_boxes[:, [1, 0, 3, 2]] * feature_maps[i].shape[-1]],
...
def coordinates_roi(inputs, pool_size, image_shape):
    ...
    pooled_features = torchvision.ops.roi_align(
        cooridnates, [boxes[:, [1, 0, 3, 2]] * cooridnates.shape[-1]],
        output_size=(pool_size, pool_size))
...
def detection_target_layer(proposals, gt_class_ids, gt_boxes, gt_masks, gt_parameters, config):
    ...
    if positive_roi_bool.sum() > 0:
        ...
        # print(box_ids)
        # print('check also box ids')
        # print(roi_masks.shape)
        # print('check multiplication with this shape')
        # exit()

        if config.NUM_PARAMETER_CHANNELS > 0:
            cropped = torchvision.ops.roi_align(roi_masks[:, :, :, 0].unsqueeze(1),
                                                [boxes[:, [1, 0, 3, 2]] * roi_masks.shape[-2]],
                                                output_size=config.MASK_SHAPE)
                # CropAndResizeFunction(config.MASK_SHAPE[0], config.MASK_SHAPE[1], 0)\
                #           (roi_masks[:, :, :, 0].contiguous().unsqueeze(1), boxes, box_ids).data, requires_grad=False
            masks = Variable(cropped.data, requires_grad=False).squeeze(1)
            masks = torch.round(masks)
            cropped = torchvision.ops.roi_align(roi_masks[:, :, :, 1].unsqueeze(1),
                                                [boxes[:, [1, 0, 3, 2]] * roi_masks.shape[-2]],
                                                output_size=config.MASK_SHAPE)
            # CropAndResizeFunction(config.MASK_SHAPE[0],
            #                                                     config.MASK_SHAPE[
            #     1], 0)(roi_masks[:, :, :, 1].contiguous().unsqueeze(1), boxes, box_ids).data, requires_grad=False
            parameters = Variable(cropped.data, requires_grad=False).squeeze(1)
            masks = torch.stack([masks, parameters], dim=-1)
        else:
            cropped = torchvision.ops.roi_align(roi_masks.unsqueeze(1),
                                                [boxes[:, [1, 0, 3, 2]] * roi_masks.shape[-2]],
                                                output_size=config.MASK_SHAPE)
            # cropped = CropAndResizeFunction(config.MASK_SHAPE[0], config.MASK_SHAPE[1], 0)(roi_masks.unsqueeze(1), boxes, box_ids)
            masks = Variable(cropped.data, requires_grad=False).squeeze(1)
            masks = torch.round(masks)            
            pass
...
class Depth(nn.Module):
    ...
    def forward(self, feature_maps):
        ...
        if self.crop:
            ...
            zeros = torch.zeros((len(x), self.num_output_channels, 80, 640)).to(device)
        ...
        if mode == 'inference':
            ...
            if len(detections) > 0:
                ...
                if len(detections) > 0:
                    ...
                    if self.config.GPU_COUNT:
                        box_ids = box_ids.to(device)
                    roi_gt_masks = torchvision.ops.roi_align(
                        roi_gt_masks.unsqueeze(1),
                        [boxes[:, [1, 0, 3, 2]] * roi_gt_masks.shape[-2]],
                        output_size=self.config.FINAL_MASK_SHAPE)
                    roi_gt_masks = Variable(roi_gt_masks.data, requires_grad=False)
